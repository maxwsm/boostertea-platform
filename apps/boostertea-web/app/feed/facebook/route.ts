...
export async function GET() {
  try {
    const products = await prisma.product.findMany({
      where: {
        brand: { slug: 'boostertea' },
      },
    });

    const domain = 'https://boostertea.com.ua';
    ...
    const itemsXml = products
      .map((product) => {
        ...
        // Facebook allows standard Google tags but has some specific recommendations
        return `
    <item>
      <g:id>${id}</g:id>
...
<rss version="2.0" xmlns:g="http://base.google.com/ns/1.0">
  <channel>
    <title>BoosterTea - Facebook Catalog</title>
    <link>${domain}</link>
    <description>Офіційний каталог продукції BoosterTea для Meta Ads</description>
${itemsXml}
  </channel>
</rss>`;
