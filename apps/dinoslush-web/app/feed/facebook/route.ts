...
export async function GET() {
  try {
    const products = await prisma.product.findMany({
      where: {
        brand: { slug: 'dinoslush' },
      },
    });

    const domain = 'https://dinoslush.com.ua';
...
<rss version="2.0" xmlns:g="http://base.google.com/ns/1.0">
  <channel>
    <title>DinoSlush - Facebook Catalog</title>
    <link>${domain}</link>
    <description>Офіційний каталог продукції DinoSlush для Meta Ads</description>
${itemsXml}
  </channel>
</rss>`;
